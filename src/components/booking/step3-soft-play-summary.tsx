...
import type { SoftPlayBookingDetails, Promotion } from '@/lib/types';
import { format } from 'date-fns';
import { Badge } from '@/components/ui/badge';
import { Separator } from '@/components/ui/separator';
import { Users, Calendar } from 'lucide-react';

interface Step3SoftPlayProps {
...
export function Step3_SoftPlay_Summary({ bookingDetails, basePrice, discountAmount, finalPrice, promotion }: Step3SoftPlayProps) {
  const { activityName, adults, children, date, time, contactDetails } = bookingDetails;
      ...
      <div className="space-y-4 rounded-lg border bg-muted/20 p-4">
        ...
        <div className="space-y-2 text-sm">
          <div className="flex justify-between">
            <span>Base price ({children} x £{(basePrice/children).toFixed(2)}):</span>
